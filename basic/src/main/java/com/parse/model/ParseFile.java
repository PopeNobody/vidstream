...
import com.parse.callback.GetDataStreamCallback;
import com.parse.callback.GetFileCallback;
...
public class ParseFile implements Parcelable {
  ...
  public ParseFile(File file, byte[] data, String contentType) {
    this((file == null ? null : file.getName()), data, contentType);
  }
  ...
  public ParseFile(byte[] data, String contentType) {
    this((String)null, data, contentType);
  }
   ...
   */
  public ParseFile(Parcel source, ParseParcelDecoder decoder) {
    this(new State.Builder().url(source.readString()).name(source.readString())
                            .mimeType(source.readByte() == 1 ? source.readString() : null).build());
  }
  ...
  private Task<Void> saveAsync(
      final String sessionToken, final ProgressCallback uploadProgressCallback, Task<Void> toAwait,
  ...
  )
  {
    ...
    return toAwait.continueWithTask(new Continuation<Void, Task<Void>>() {
      ...
      public Task<Void> then(Task<Void> task) {
        ...
        if (data != null) {
          saveTask = getFileController().saveAsync(state, data, sessionToken,
                                                   progressCallbackOnMainThread(uploadProgressCallback),
                                                   cancellationToken);
        } else {
          saveTask = getFileController().saveAsync(state, file, sessionToken,
                                                   progressCallbackOnMainThread(uploadProgressCallback),
                                                   cancellationToken);
        }
  ...
  public Task<Void> saveAsync(
      final String sessionToken, final ProgressCallback uploadProgressCallback, final Task<Void> cancellationToken
  )
  {
  ...
  public Task<InputStream> getDataStreamInBackground(final ProgressCallback progressCallback) {
    ...
    return taskQueue.enqueue(new Continuation<Void, Task<InputStream>>() {
      ...
      public Task<InputStream> then(Task<Void> toAwait) {
        return fetchInBackground(progressCallback, toAwait, cts.getTask()).onSuccess(
            new Continuation<File, InputStream>() {
              @Override
  ...
  private Task<File> fetchInBackground(
      final ProgressCallback progressCallback, Task<Void> toAwait, final Task<Void> cancellationToken
  )
  {
    if (cancellationToken != null && cancellationToken.isCancelled()) {
      return Task.cancelled();
    ...
    return toAwait.continueWithTask(new Continuation<Void, Task<File>>() {
      ...
      public Task<File> then(Task<Void> task) {
        ...
        return getFileController().fetchAsync(state, null, progressCallbackOnMainThread(progressCallback),
                                              cancellationToken);
      }
    });
  }
  ...
  /* package */ public static class State {
