...
public class ParseCommandCache extends ParseEventuallyQueue {
  private static final String TAG = "com.parse.rest.ParseCommandCache";
  // Lock guards access to the file system and all the instance variables above. It is static so
  ...
  // on runningLock if he is sure the value of running is going to change. Only the run loop
  ...
  ConnectivityNotifier.ConnectivityListener listener = (context, intent) -> {
    final boolean connectionLost = intent.getBooleanExtra(ConnectivityManager.EXTRA_NO_CONNECTIVITY, false);
  ...
  private static File getCacheDir() {
    ...
    @SuppressWarnings("deprecation") File cacheDir = new File(Parse.getParseDir(), "CommandCache");
   ...
   * but com.parse can't be reached, waits timeoutRetryWaitSeconds before retrying up to
  ...
  private void maybeRunAllCommandsNow(int retriesRemaining) {
    synchronized (lock) {
      ...
      for (String fileName : fileNames) {
        ...
        final TaskCompletionSource<JSONObject> tcs = pendingTasks.getOrDefault(file, null);
            ...
            commandTask = command.executeAsync(httpClient)
                                 .continueWithTask(task -> {
                                   ...
                                   if (tcs != null) {
                                     ...
                                     String objectId = json1.optString("objectId", "");
                                     if (objectId.isEmpty()) {
                                       ParseCorePlugins.getInstance().getLocalIdManager()
