...
public class EventuallyPin extends ParseObject {
  ...
  )
  {
    ...
    if (command.httpPath.startsWith("classes")) {
      if (command.method == ParseHttpMethod.POST || command.method == ParseHttpMethod.PUT) {
        type = TYPE_SAVE;
      } else if (command.method == ParseHttpMethod.DELETE) {
        type = TYPE_DELETE;
      }
    } else {
  ...
  private static Task<EventuallyPin> pinEventuallyCommand(
      int type, ParseObject obj, String operationSetUUID, String sessionToken, JSONObject command
  )
  {
  ...
  public static Task<List<EventuallyPin>> findAllPinned(Collection<String> excludeUUIDs) {
    ParseQuery<EventuallyPin> query = new ParseQuery<>(EventuallyPin.class).fromPin(PIN_NAME).ignoreACLs()
